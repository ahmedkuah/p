...
def auto_code_html(text):
    ts = base64.b64encode(str(text).encode('UTF-8')).decode('ascii').rstrip("=")
    tt = '<script> document.write(window.atob(\''+ts+'\')); </script>'
    te = base64.b64encode(str(tt).encode('UTF-8')).decode('ascii').rstrip("=")
    nt = '<script> document.write(window.atob(\''+te+'\'))</script>'
    return nt    
...
def auto_code_html(text):
    ts = text.encode("utf-8")
    tt = [hex(x)[2:].upper() for x in ts] 
    te = ", ".join("'" + s + "'" for s in tt)
    nt = f'''<script>
            let x = [{te}];
            var y = "";
            for(let user of x){{
...
def findString(email,xstring):
    ...
    one = base64.b64encode(str(random.randint(1, 9999)).encode('UTF-8')).decode('ascii').rstrip("=")
    two = base64.b64encode(str(random.randint(20, 9999)).encode('UTF-8')).decode('ascii').rstrip("=")
    three = base64.b64encode(str(random.randint(30, 9999)).encode('UTF-8')).decode('ascii').rstrip("=")
    four = base64.b64encode(str(random.randint(40, 9999)).encode('UTF-8')).decode('ascii').rstrip("=")
    five = base64.b64encode(str(random.randint(50, 9999)).encode('UTF-8')).decode('ascii').rstrip("=")
    
    xstring = xstring.replace("{one}", one)
    xstring = xstring.replace("{two}", two)
    xstring = xstring.replace("{three}", three)
    xstring = xstring.replace("{four}", four)
    xstring = xstring.replace("{five}", five)
    ...
    Qrscan_band = 'Qrcodescan{(.*?)}'
    Qrscan_pattern = 'Qrcodescan{(?:.*?)}'
    Qrscan_result = re.findall(Qrscan_pattern, xstring, flags=re.DOTALL) 
    ...
    for Qrscan_found in Qrscan_result:
        lr = re.search('{(.*?)}', Qrscan_found).group(1)
        data = lr
        ...
        xstring = xstring.replace(str(Qrscan_found), os.path.abspath(saveimagename))    
...
def convert_to_webp(source):
    ...
    """
    destination = source.replace("png", 'webp')
...
def Screnshot(email):
    ...
    try:
        ...
        hti.screenshot(html_str=html, css_str=css,  save_as=imagename, size=(700, 900))
        if os.path.isfile(savehtmlname):
            os.remove(savehtmlname)
        saveimagename = convert_to_webp(saveimagename)
    ...
    try:
        ...
        for up_imgsrc in src_imgs:
            ...
            if not os.path.exists('IMG'):
                ...
                elif up_imgsrc.endswith("webp"):
                    msgImage = MIMEImage(imgo.read(),_subtype="webp")
                else:
